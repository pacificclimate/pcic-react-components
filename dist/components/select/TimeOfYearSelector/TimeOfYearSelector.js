...
import SelectWithValueReplacement from '../SelectWithValueReplacement'; // `isInvalidValue` examines the *current options list* to determine whether
...
const isInvalidValue = options => cond([[isNull, constant(false)], // null = no selection
[isUndefined, constant(true)], // undefined = 'replace me'
[otherwise, option => find({
  value: option.value
})(options).isDisabled]]); // `replaceInvalidValue` returns the first enabled option in the options list,
...
export default class TimeOfYearSelector extends React.Component {
  render() {
    ...
    return React.createElement(SelectWithValueReplacement, _extends({
      options: options,
